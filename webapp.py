...
st.write(
    ...
    **Mean Reversion by: Fernando Nogueira**  \n
    (30 SEC. PARA RODAR -  data manipulation on time)
...
dfc = fb.year_filter(fb.mean_reversion(df, n_std=dp_lst, window=window_choices), year)
buy_p, sell_p, ret = fb.strategy(dfc)
dfc['BUY_POINT'] = buy_p
dfc['SELL_POINT'] = sell_p
profit_ratio_number = fb.profit_ratio(buy_p, sell_p)
string_profit_ratio = '{:.2f} %'.format(profit_ratio_number * 100)
ipca_string = '{:.2f} %'.format(ipca_choices * 100)

fig1, ax1 = plt.subplots(figsize=(12, 6))
ax1.plot(dfc['Close'], label='PRICE')
ax1.plot(dfc['MEAN'], label='MEAN')
ax1.plot(dfc['MEAN_MORE_STD'], label='MEAN + STD')
ax1.plot(dfc['MEAN_LESS_STD'], label='MEAN - STD')
ax1.plot(dfc.index, buy_p, marker = '^', markersize = 5, color = 'green', label = 'BUY SIGNAL')
ax1.plot(dfc.index, sell_p, marker = 'v', markersize = 5, color = 'r', label = 'SELL SIGNAL')
